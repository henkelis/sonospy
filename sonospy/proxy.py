...
class Proxy(object):
    ...
    def _add_services(self):
        ...
        self.cdservice = ContentDirectory(self.root_device.location, self, self.webserverurl, self.wmpurl, self.dbspec, self.wmpudn)
...
#       result is manually created from children
    ...
    def load_user_index_flag(self):
        ...
        try:
            ini_user_indexes = self.proxy.config.get('user indexes', 'user_indexes')
...
class ContentDirectory(Service):
    ...
    def __init__(self, proxyaddress, proxy , webserverurl, wmpurl, dbspec, wmpudn):
        ...
        if not self.user_indexes:
            ...
            # create MediaServer with default hierarchical ID
            self.mediaServer = MediaServer(self.proxy, self.dbspec, 'UPNP', 'HIERARCHY_DEFAULT', self.proxyaddress, self.webserverurl, self.wmpurl)

        else:

            # create MediaServer with user defined hierarchical ID
            self.mediaServer = MediaServer(self.proxy, self.dbspec, 'UPNP', 'HIERARCHY', self.proxyaddress, self.webserverurl, self.wmpurl)
