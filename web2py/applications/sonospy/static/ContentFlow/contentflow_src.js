...
var ContentFlowItem  = function (CFobj, element, index) {
    ...
    this.clickItem = function (event) {
        ...
        if (this._activeItem == item) {
            this.conf.onclickActiveItem(item, event, el);
        }
        else {
    ...
    this.mouseoverItem = function (event) {
        if(!event) var event = window.event;
        var el = event.target ? event.target : event.srcElement;
        var index = el.itemIndex ? el.itemIndex : el.parentNode.itemIndex;
        var item = this.items[index];
        if (this._activeItem == item) {
            this.conf.onmouseoverActiveItem(item, event, el);
        }
        else {
//            if (this.conf.onhoverInactiveItem(item) != false ) this.moveToIndex(index);
        }
    }.bind(CFobj),

    this.mouseoutItem = function (event) {
        if(!event) var event = window.event;
        var el = event.target ? event.target : event.srcElement;
        var index = el.itemIndex ? el.itemIndex : el.parentNode.itemIndex;
        var item = this.items[index];
        if (this._activeItem == item) {
            this.conf.onmouseoutActiveItem(item, event, el);
        }
        else {
//            if (this.conf.onhoverInactiveItem(item) != false ) this.moveToIndex(index);
        }
    ...
    this.mousemoveItem = function (event) {
        if(!event) var event = window.event;
        var el = event.target ? event.target : event.srcElement;
        var index = el.itemIndex ? el.itemIndex : el.parentNode.itemIndex;
        var item = this.items[index];
        if (this._activeItem == item) {
            this.conf.onmousemoveActiveItem(item, event, el);
        }
        else {
//            if (this.conf.onhoverInactiveItem(item) != false ) this.moveToIndex(index);
        }
...
ContentFlowItem.prototype = {
    ...
    initHover: function () {
        var overItem = this.mouseoverItem;
        var outItem = this.mouseoutItem;
        var moveItem = this.mousemoveItem;
        this[this._activeElement].addEvent('mouseover', overItem, false);
        this[this._activeElement].addEvent('mouseout', outItem, false);
        this[this._activeElement].addEvent('mousemove', moveItem, false);
    },
...
*/
    ...
    /*
     * initialises flows - use if flows have been added since doc load and
     * there was no content flow beforehand
     * - otherwise reflection does not work
     * (copied from ContentFlowGlobal.init()
     */
    preinit: function () {
        var divs = document.getElementsByTagName('div');
        DIVS: for (var i = 0; i < divs.length; i++) {
...
//        console.log("size h: ", size.height, " w: ", size.width);
//        console.log("item h: ", item.size.height, " w: ", item.size.width);
